    ...
    <section id="testimoni" className="py-24 bg-primary-light scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Kata Alumni Kami</h2>
          <div className="w-20 h-1.5 bg-primary mx-auto rounded-full mb-6"></div>
          <p className="text-lg text-gray-600">
            Kisah sukses mereka adalah bukti nyata komitmen kami terhadap kualitas pendidikan.
          </p>
        </div>
          ...
          {testimonials.map((testi, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mt-auto">
                <img src={testi.avatar} alt={testi.name} className="w-12 h-12 rounded-full border-2 border-primary-light" />
                <div>
                  <h4 className="font-bold text-gray-900">{testi.name}</h4>
                  <p className="text-sm text-primary font-medium">{testi.program}</p>
                </div>
              </div>
            </motion.div>
          ))}
