...
import { MessageCircle, PenTool, ClipboardList, BookOpen, Presentation, Users, FileCheck, PlaneTakeoff, Compass } from 'lucide-react';
    ...
    <section className="py-24 bg-neutral-gray relative scroll-mt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-4 font-sans tracking-tight">Alur Keberangkatan Resmi</h2>
          <div className="w-20 h-1.5 bg-primary mx-auto rounded-full mb-6"></div>
          <p className="text-lg text-gray-600 leading-relaxed font-medium">
            Proses sistematis bimbingan terintegrasi dari mulai konsultasi gratis hingga Anda siap berangkat dan bekerja di Jepang.
          </p>
        </div>
            ...
            {steps.map((step, idx) => (
              ...
              >
                ...
                <div className="w-full lg:w-[45%]">
                  <div className={`bg-white p-8 rounded-[2rem] shadow-sm hover:shadow-xl border border-gray-100/80 flex flex-col sm:flex-row gap-5 hover:-translate-y-1.5 transition-all duration-300 relative group overflow-hidden ${
                    idx % 2 === 0 ? 'lg:text-right lg:flex-row-reverse' : 'lg:text-left'
                  }`}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2 justify-center sm:justify-start lg:justify-start lg:group-hover:text-primary transition-colors">
                        <span className="lg:hidden text-xs font-bold text-primary bg-primary-light px-2.5 py-1 rounded-full">{idx + 1}</span>
                        <h4 className="font-bold text-xl text-gray-900 group-hover:text-primary transition-colors">{step.title}</h4>
                      </div>
